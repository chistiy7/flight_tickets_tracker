...
def compute_cost(legs: list[Leg], config: Config) -> CostBreakdown:
    ...
    # Включённое в пакет (отель, обратное плечо) не снижает его цену, а идёт
    # отдельным зачётом — и только в той части, которая вам действительно нужна.
    breakdown.bundle_credit_rub = sum(leg.bundle_credit_rub for leg in legs)
